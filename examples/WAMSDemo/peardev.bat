...
FOR %%x IN ("C:\phpsites\WAMSDemo/PEAR") DO (if %%x=="%PMTPMTPMT%" GOTO :NOTINSTALLED)

REM Check PEAR global ENV, set them if they do not exist
IF "%PHP_PEAR_INSTALL_DIR%"=="" SET "PHP_PEAR_INSTALL_DIR=C:\phpsites\WAMSDemo/PEAR"
IF "%PHP_PEAR_BIN_DIR%"=="" SET "PHP_PEAR_BIN_DIR=C:\phpsites\WAMSDemo"
IF "%PHP_PEAR_PHP_BIN%"=="" SET "PHP_PEAR_PHP_BIN=c:\program files (x86)\iis express\php\v5.4\php.exe"
GOTO :INSTALLED
